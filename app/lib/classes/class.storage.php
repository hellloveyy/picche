...
class Storage {
	...
	public static function insert($values) {
		try {
			...
			if($storage_api == 'gcloud') {
				// Todo: Allow this kind of key: -----BEGIN PRIVATE KEY-----
				// IF NOT, then json_decode(<secret>) and walk the array until the first -----BEGIN PRIVATE KEY----- match;
	...
	public static function update($id, $values) {
		try {
			...
			if($values['api_id'] == 2) {
				$json = json_decode($values['secret'], TRUE);
				foreach($json as $k => $v) {
						if(self::looksLikePEM($v)) {
								$key = $v;
								break;
						}
				}
	...
	*/
	...
	private static function looksLikePEM(string $var) {
			return (strpos($var, '-----BEGIN ') !== FALSE);
	}
